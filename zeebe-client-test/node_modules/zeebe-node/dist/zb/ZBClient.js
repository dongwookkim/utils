...
class ZBClient extends events_1.EventEmitter {
    ...
    async deployWorkflow(workflow) {
        ...
        if (workFlowRequests.length > 0) {
            return this.executeOperation(() => this.gRPCClient.deployWorkflowSync({
    ...
    publishMessage(publishMessageRequest) {
        return this.executeOperation(() => this.gRPCClient.publishMessageSync(lib_1.stringifyVariables(publishMessageRequest)));
    }
    /**
    ...
    updateJobRetries(updateJobRetriesRequest) {
        return this.executeOperation(() => this.gRPCClient.updateJobRetriesSync(updateJobRetriesRequest));
    }
    failJob(failJobRequest) {
        return this.executeOperation(() => this.gRPCClient.failJobSync(failJobRequest));
    }
    ...
    createWorkflowInstance(configOrbpmnProcessId, variables) {
        ...
        return this.retry === true
            ? this.executeOperation(() => this.gRPCClient.createWorkflowInstanceSync(lib_1.stringifyVariables(createWorkflowInstanceRequest)))
    ...
    createWorkflowInstanceWithResult(configOrBpmnProcessId, variables) {
        ...
        return this.retry === true
            ? this.executeOperation(() => this.gRPCClient.createWorkflowInstanceWithResultSync({
                fetchVariables,
    ...
    async cancelWorkflowInstance(workflowInstanceKey) {
        ...
        return this.executeOperation(() => this.gRPCClient.cancelWorkflowInstanceSync({
            workflowInstanceKey,
        }));
    }
    setVariables(request) {
        ...
        */
        ...
        return this.executeOperation(() => this.gRPCClient.setVariablesSync(request));
    }
    resolveIncident(incidentKey) {
        return this.executeOperation(() => this.gRPCClient.resolveIncidentSync(incidentKey));
    }
    constructGrpcClient({ onReady, onConnectionError, tasktype, }) {
     ...
     */
    async executeOperation(operation, retries) {
        return this.retry
     ...
     */
    async retryOnFailure(operation, retries = this.maxRetries) {
        return promise_retry_1.default((retry, n) => {
            if (this.closing || this.gRPCClient.channelClosed) {
                return Promise.resolve();
            }
            if (n > 1) {
                this.logger.error(`gRPC connection is in failed state. Attempt ${n}. Retrying in 5s...`);
            }
