...
class GRPCClient extends events_1.EventEmitter {
    constructor({ basicAuth, connectionTolerance, host, loglevel, oAuth, options = {}, packageName, protoPath, service, namespace, tasktype, useTLS, stdout = console, onConnectionError, onReady, }) {
        super();
        this.channelClosed = false;
        this.connected = false;
        this.gRPCRetryCount = 0;
        this.handleGrpcError = (stream) => async (err) => {
            this.emit('error', err);
            this.logger.error(`GRPC ERROR: ${err.message}`);
            const channelState = await this.watchGrpcChannel();
            this.logger.debug(`gRPC Channel state: ${connectivityState[channelState]}`);
        ...
        const proto = grpc_1.loadPackageDefinition(this.packageDefinition)[packageName];
        const listMethods = this.packageDefinition[`${packageName}.${service}`];
        const channelCredentials = useTLS
            ? grpc_1.credentials.createSsl()
        ...
        for (const key in listMethods) {
            if (listMethods[key]) {
                ...
                this[`${methodName}Stream`] = async (data) => {
                     ...
                     */
                    stream.on('error', (error) => this.handleGrpcError(stream)(error));
                    ...
                    return new Promise(async (resolve, reject) => {
                        try {
                            ...
                            client[methodName](data, metadata, (err, dat) => {
                                ...
                                if (err) {
                                    const code = err.code;
                                    const isNetworkError = code === GrpcError.UNAVAILABLE;
                                    if (isNetworkError) {
                                        this.setNotReady();
                                    }
                                    else {
    ...
    close(timeout = 5000) {
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                alreadyClosed = msg.includes('Cannot call getConnectivityState on a closed Channel');
                if (alreadyClosed) {
                    setTimeout(() => resolve(), 2000);
                }
            }
            if (!alreadyClosed) {
                this.logger.info(`GRPC Channel State: ${connectivityState[state]}`);
                const deadline = new Date().setSeconds(new Date().getSeconds() + 300);
                gRPC.getChannel().watchConnectivityState(state, deadline, async () => {
                    try {
                        const newState = gRPC
                            .getChannel()
                            .getConnectivityState(false);
                        this.logger.info(`GRPC Channel State: ${connectivityState[newState]}`);
                    }
                    catch (e) {
                        const msg = e.toString();
                        alreadyClosed = msg.includes('Cannot call getConnectivityState on a closed Channel');
    ...
    watchGrpcChannel() {
        return new Promise(resolve => {
            ...
            gRPC.getChannel().watchConnectivityState(state, deadline, async (error) => {
                if (this.channelClosed) {
                    return;
                }
                this.gRPCRetryCount++;
                ...
                const newState = gRPC
                    .getChannel()
                    .getConnectivityState(false);
                this.logger.log(`gRPC Channel State: ${connectivityState[newState]}`);
